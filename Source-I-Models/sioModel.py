...
class SiOModel:
    ...
    def v_phi_dropoff(self, r):
        factor = np.where(r > self.r_outer(0.0), 1 - ((r - self.r_outer(0.0)) / (self.r_outer(1000.0) - self.r_outer(0.0))), 1.0)
    ...
    def v_phi_dropoff(self, r, z):
        mask = np.where(r > self.r_outer(0.0))
        dropoff = np.where(mask, -(self.r_outer(100.0) /(self.r_outer(0.0) - self.r_outer(50.0))) + r / (self.r_outer(0.0) - self.r_outer(50.0)), 0.0)
        return dropoff
    
    
    def v_phi_dropoff(self, r, z):
        mask = (r > self.r_inner(z)) & (r < self.r_outer(z))
        factor = np.where(mask, 1 - ((r - self.r_outer(0.0)) / (self.r_outer(1000.0) - self.r_outer(0.0))), 1.0)
        dropoff = np.where(factor > 0.0, factor, 0.0)
        return dropoff
    ...
    def velocity_model_field(self, r, phi, zprime):
        ...
        vphi = np.where(np.abs(zprime) < self.z_no_rotation, self.v_phi0 * self.v_phi_dropoff(r, np.abs(zprime)), 0.)
    ...
    def velocity_rotation_graph(self):
        ...
        x, y = np.meshgrid(np.linspace(-400,400,501),
                            np.linspace(-400, 400, 501))
        velocity_rotational_inner_radius = []
        velocity_rotational_outer_radius = []
        for z in z_array:
            if np.abs(z)<self.z_no_rotation:
                r_in=self.r_inner(z)
                r_out=self.r_outer(z)
                velocity_rotational_inner_radius.append(self.v_phi0*self.v_phi_dropoff(r_in, z))
                velocity_rotational_outer_radius.append(self.v_phi0*self.v_phi_dropoff(r_out, z))
            else:
        ...
        ax.scatter(z_array, velocity_rotational_inner_radius, label = 'R_in')
        ax.scatter(z_array, velocity_rotational_outer_radius, label='R_out')
    ...
    def specific_angular_momentum(self):
        ...
        x, y = np.meshgrid(np.linspace(-400,400,501),
                            np.linspace(-400, 400, 501))
        specific_angular_momentum_inner_radius = []
        specific_angular_momentum_outer_radius = []
        vir = []
        rin = []
        rot = []
        vor = []
        ...
        ax.scatter(z_array, specific_angular_momentum_inner_radius, label = 'R_in')
        ax.scatter(z_array, specific_angular_momentum_outer_radius, label='R_out')

        ax.set_xlabel('z(au)')
        ax.set_ylim(bottom=0)
        ax.set_ylabel('Specific Angular Momentum(au km s^-1)')
        ax.legend()
    ...
    def position_velocity(self):
        ...
        vmin = SiOModel.v_lsr - 20.
        vmax = SiOModel.v_lsr + 20.
        nvbins = 50
